...
[**get_hos_logs**](SamsaraApi.md#get_hos_logs) | **GET** /fleet/hos/logs | Get last known HOS log entry
[**get_hos_logs_history**](SamsaraApi.md#get_hos_logs_history) | **GET** /fleet/hos/logs/history | Get historical HOS logs.
 ...
 **equipment_ids** | [**list[str]**](str.md)| A filter on the data based on this comma-separated list of equipment IDs. Example: &#x60;equipmentIds&#x3D;1234,5678&#x60; | [optional] 
    ...
    # **get_hos_logs**
    > HosLogsSnapshotResponse get_hos_logs(tag_ids=tag_ids, parent_tag_ids=parent_tag_ids, driver_ids=driver_ids, time=time, after=after)

    Get last known HOS log entry

      Returns the last known HOS log entry for the requested time.  By default, this endpoint will return the most recent HOS log entry for all drivers. The `time` parameter allows you to request an HOS log entry \"snapshot\" - i.e., the HOS log entry at that time. See the HOS logs [history](#getHosLogsHistory) endpoint to get HOS logs for a time *period*.  You can filter the results using the driver ID or tag ID parameters.  **Note:** If data is still being uploaded from the Samsara Driver App, it may not be completely reflected in the response from this endpoint.
...
try:
    # Get last known HOS log entry
    api_response = api_instance.get_hos_logs(tag_ids=tag_ids, parent_tag_ids=parent_tag_ids, driver_ids=driver_ids, time=time, after=after)
  pprint(api_response)
except ApiException as e:
 ...
 **after** | **str**| If specified, this should be the endCursor value from the previous page of results. When present, this request will return the next page of results that occur immediately after the previous page of results. | [optional] 
    ...
    [**HosLogsSnapshotResponse**](HosLogsSnapshotResponse.md)
    ...
    # **get_hos_logs_history**
    > HosLogsHistoryResponse get_hos_logs_history(start_time, end_time, tag_ids=tag_ids, parent_tag_ids=parent_tag_ids, driver_ids=driver_ids, after=after)

    Get historical HOS logs.

      Returns all known HOS (hours of service) logs during the given time range for all drivers. This can be optionally filtered by tags or specific driver IDs or driver external IDs.  **Note:** If  data is still being uploaded from the Samsara Driver App, it may not be completely reflected in the response from this endpoint. The best practice is to wait a couple of days before querying this endpoint to make sure that all data from the Samsara Driver App has been uploaded.
...
try:
    # Get historical HOS logs.
    api_response = api_instance.get_hos_logs_history(start_time, end_time, tag_ids=tag_ids, parent_tag_ids=parent_tag_ids, driver_ids=driver_ids, after=after)
  pprint(api_response)
except ApiException as e:
print("Exception when calling SamsaraApi->get_hos_logs_history: %s\n" % e)
```
 ...
 **after** | **str**| If specified, this should be the endCursor value from the previous page of results. When present, this request will return the next page of results that occur immediately after the previous page of results. | [optional] 
    ...
    [**HosLogsHistoryResponse**](HosLogsHistoryResponse.md)
    ...
    - **Accept**: application/json
      ...
      | Status code | Description | Response headers |
      |-------------|-------------|------------------|
        **200** | List of all HOS logs for the specified drivers and time range. |  -  |
        **0** | Error response |  -  |
 ...
 **vehicle_ids** | [**list[str]**](str.md)| A filter on the data based on this comma-separated list of vehicle IDs. Example: &#x60;vehicleIds&#x3D;1234,5678&#x60; | [optional] 
    ...
    List most recent vehicle stats
      ...
      Returns last known stats for the given time for all vehicles (connected via Samsara Vehicle Gateways). If no time is specified the current time is used. This can be optionally filtered by tags or specific vehicle IDs. See [here](https://developers.samsara.com/docs/vehicle-stats) for more details.
