...
class SamsaraApi(object):
    ...
    def get_vehicle_stats_feed(self, types, **kwargs):  # noqa: E501
        ...
        :param list[str] tag_ids: A filter on the data based on this comma-separated list of tag IDs. Example: `tagIds=1234,5678`
        :param list[str] vehicle_ids: A filter on the data based on this comma-separated list of vehicle IDs. Example: `vehicleIds=1234,5678`
        :param list[str] decorations: The decoration stat types that you would like to annotate each of the primary stat responses with. For example, if you wish to know the vehicle's location whenever the engine changed states, you may set types=engineStates&decorations=gps. Currently only two decorations per request are supported. Note that decorations will significantly increase the response payload size.  - `engineStates`: The state of the engine (`Off`, `On`, `Idle`). - `faultCodes`: The diagnostic fault codes for the vehicle. - `fuelPercents`: The engine fuel level in percentage points (e.g. `99`, `50`, etc). - `engineRpm`: The revolutions per minute of the engine. - `obdOdometerMeters`: The odometer reading according to on-board diagnostics. If Samsara does not have diagnostic coverage for a particular vehicle, the value for this stat type will be omitted. - `obdEngineSeconds`: The cumulative number of seconds the engine has run according to on-board diagnostics. - `ecuSpeedMph`: The speed of the engine in miles per hour according to the ECU. - `gpsDistanceMeters`: The distance the vehicle has traveled since the gateway was installed based on GPS calculations. - `batteryMilliVolts`: The vehicle battery voltage reading. - `auxInput1`: Stat events from the [auxiliary input 1](https://kb.samsara.com/hc/en-us/articles/232232368-Auxiliary-Inputs) for the vehicle. For more details see the `data.auxInputType1` field in the response body for [Retrieving a Vehicle](#operation/getVehicle). - `auxInput2`: Stat events from the [auxiliary input 2](https://kb.samsara.com/hc/en-us/articles/232232368-Auxiliary-Inputs) for the vehicle. For more details see the `data.auxInputType2` field in the response body for [Retrieving a Vehicle](#operation/getVehicle). - `auxInput3-10`: Stat events from the [auxiliary inputs](https://kb.samsara.com/hc/en-us/articles/232232368-Auxiliary-Inputs) for the vehicle. For more details see the `data.auxInputType3-10` fields in the response body for [Retrieving a Vehicle](#operation/getVehicle). Note, ports 3-10 are only available on gateways with the aux expander. - `nfcCardScans`: Nfc cards scan data. -  `gps`: GPS location of the vehicle.
        :param _preload_content: if False, the urllib3.HTTPResponse object will
                                 be returned without reading/decoding response
                                 data. Default is True.
        :param _request_timeout: timeout setting for this request. If one
